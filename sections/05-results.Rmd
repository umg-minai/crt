...
Arrow3 and Arrow5 had longer median CRT-free periods of `r .as.word(median(m$svf["ManLu=Arrow3"]))` and `r .as.word(median(m$svf["ManLu=Arrow5"]))` days, respectively (Table \@ref(tab:tablemedsurv)).
...
However, the median (IQR) largest dimension of the thrombosis was almost identical across all examinations: `r .medLU(m$data$HeightMax)` mm and `r .medLU(m$data$WidthMax)` mm, respectively (Supplemental eFigure \@ref(fig:crtheightbp) to \@ref(fig:crtwidthovertime)).
...
However, the results don't vary much even when all variables are included (Supplemental eTable \@ref(tab:supplhrtablefull) and eFigure \@ref(fig:forestplotfull)).

As shown in the forest plot (Figure \@ref(fig:forestplot)), the Arrow4 and Vygon5 CVC types were significantly associated with higher rates of CRT.
