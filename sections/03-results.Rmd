...
However, `r s <- sum(m$data$Complications == "symptomatic thrombosis", na.rm = TRUE); .cli_text("{(.as.word(s))} symptomatic {qty(s)}thrombos{?is/es}")` was observed
...
However, the results didn't change much even when we included all the variables (see Supplement for a regression with half the number of samples but all the variables).
...
Cancer was also associated with CRT (hazard ratio `r format(exp(rgcx$coefficients["AnticoagulationGroupNone"]), digits = 2)`), but is an inhomogeneous category of disease.
