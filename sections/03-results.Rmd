...
`r sum(!duplicated(cvc$CaseId))` consecutive criticaly ill patients
were eligible to participate in the study.
We enrolled `r sum(!cvc$isExcluded)` patients
(see supplement \@ref(tab:lottable) for a complete list)
...
As the entry of the catheter into the femoral or the subclavian vene was often difficult to visualize with ultrasound
we focus on the internal jugular vene (IVJ) and excluded  `r sum(cvc$ExclusionReason == "Site: FV", na.rm = TRUE)` and `r sum(cvc$ExclusionReason == "Site: SCV", na.rm = TRUE)` CVCs, respectively.
In addition we had to exclude in total `r .as.word(length(grep("Lumen", cvc$ExclusionReason)) + sum(cvc$ExclusionReason == "Not inserted in-house", na.rm = TRUE))` CVCs because of the very small number of catheters of this type (e.g. `r ncvc <- .as.word(sum(cvc$ManLu == "Arrow5" & grepl("ARROWgard", cvc$Type))); .cli_text("{ncvc} CVC{?s}")` by Arrow, 5 lumens with ARROWgard Blue (1st generation) coating) or because they were inserted in a foreign hospital and we were unable to verify the time of insertion.
Direct oral anticoagulants (DOACs) are an inhomogeneous group and uncommon in our ICU.
We examined only `r s <- length(grep("DOAC", cvc$ExclusionReason)); .cli_text("{(.as.word(s))} {qty(s)}patient{?s} with {qty(s)}{?a/different} {qty(s)}DOAC{?s} and decided to exclude {qty(s)}{?this/these} {qty(s)}CVC{?s}")`.
Due to missing or unreadable LOT information we had to exclude `r sum(cvc$ExclusionReason == "Missing LOT", na.rm = TRUE)` CVCs.
Finally we retained  `r sum(!cvc$isExcluded)` first-time CVCs and patients for our analysis (see table \@ref(tab:table1)).
...
Two third of all CVCs were placed by experienced practitioners who had inserted more than 50 CVCs.
About the same amount of CVCs were placed in the ICU and outside the operating room.
We examined the catheters by ultrasound `r .tbl1("FirstExam")` hours after insertion.

More than 80% of the patients admitted to our ICU had previously undergone surgery.
One third had sepsis and around 40% had cancer.
Only `r .tbl1("DVT")` patients had a history of venous thrombosis.

Patients received prophylactic anticoagulation with low molecular weighted heparins in two third of cases, and unfractionated heparin in only a few.
...
On day one, the median (IQR) height and length were `r .medLU(m$data$Height1)` mm and `r .medLU(m$data$Width1)` mm.
However, the median (IQR) largest dimension of the thrombosis across all examinations was almost identical: `r .medLU(m$data$HeightMax)` mm and `r .medLU(m$data$WidthMax)` mm (see supplement \@ref(fig:crtheightbp) to \@ref(fig:crtwidthovertime) for detailed figures).
...
However, `r s <- sum(m$data$Complications == "symptomatic thrombosis", na.rm = TRUE); .cli_text("{(.as.word(s))} symptomatic {qty(s)}thrombos{?is/es}")` symptomatic thrombosis was observed
...
We therefore ignore all variables with missing values in order to maintain the sample size.
However, the results didn't changed much even when we included all the variables (see supplement for a regression with half the number of samples but all variables).

As shown in the survival plots the type Arrow4 and Vygon5 were associated with an CRT.
...
In addition to CVC type, no prophylactic anticoagulation prior to CVC placement was another risk factor associated with CRT, with a hazard ratio of `r format(exp(rgcx$coefficients["AnticoagulationGroupNone"]), digits = 2)`.
...
Nevertheless we didn't stratified our regression model for cancer because the chi-square values for the (un)stratified models were very similar
...
Because it is very unlikely that any of the covariates studied, except for laboratory measurements, would change over the short observation period, we didn't look for any time-dependency.
