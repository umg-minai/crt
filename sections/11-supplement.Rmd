...
```{r crtheightbp, fig.width = 15, fig.height = 10, echo = FALSE, fig.cap = "Boxplots of CRT height over time."}
...
```
```{r crtheightovertime, fig.width = 15, fig.height = 10, echo = FALSE, fig.cap = "CRT height over time."}
.crtovertime(
    droplevels(cvc[!cvc$isExcluded, c("Id", "ManLu", paste0("Height", 1:17))]),
    col = cols,
    ylab = "HEIGHT [mm]"
...
```{r crtwidthbp, fig.width = 15, fig.height = 10, echo = FALSE, fig.cap = "Boxplots of CRT width over time."}
.crtplots(
...
```
```{r crtwidthovertime, fig.width = 15, fig.height = 10, echo = FALSE, fig.cap = "CRT width over time."}
.crtovertime(
    droplevels(cvc[!cvc$isExcluded, c("Id", "ManLu", paste0("Width", 1:17))]),
    col = cols,
    ylab = "WIDTH [mm]"
...
knitr::kable(
    ...
    caption = paste0(
        ...
        "DVT, deep venous thrombosis; WBC1, white blood count on day one."
    )
)
```

```{r pha-plot, echo = FALSE, fig.width = 12, fig.height = 12, fig.cap = "Scaled Schoenfeld residuals over transformed time for each covariate. Depicted $p$-values were calculated applying chi-square independence test. CVC, central venous catheters; DVT, deep venous thrombosis; WBC1, white blood count on day one."}
ggcoxzph(pha, df = 2)
```
...
tbl_regression(rgcx, exponentiate = TRUE) |>
    modify_caption(
        paste0(
            "Hazard ratios of CRT on all N=", rgcx$n,
            " analysed central venous catheters."
        )
    )
```
...
tbl_regression(rgcx.lots, exponentiate = TRUE) |>
    modify_caption(
        paste0(
            "Hazard ratios of CRT assuming that ",
            "central venous catheters with missing LOTs where of the major type ",
            "of each manufacturer. Sample size of N=", rgcx.lots$n, "."
        )
    )
```
...
tbl_regression(rgcx.full, exponentiate = TRUE) |>
    modify_caption(
        paste0(
...
```{r forestplotlots, echo = FALSE, fig.width = 12, fig.height = 12, fig.cap = "Hazard ratios for all central venous catheters assuming missing LOTs where major types of each manufacturer. DVT, deep venous thrombosis; LMWH, low-molecular-weighted heparins; UFH, unfractionated heparin; WBC1, white blood count on day one."}
.ggforest(
    rgcx.lots,
    data = rd.lots,
    rename = rgcx.nms
...
```{r forestplotfull, echo = FALSE, fig.width = 12, fig.height = 12, fig.cap = "Hazard ratios for all available variables but with reduced sample size. CRP1: C-reactive peptide on day one; D-dimer1: d-dimer on day one; DVT, deep venous thrombosis; LMWH, low-molecular-weighted heparins; UFH, unfractionated heparin; WBC1, white blood count on day one."}
.ggforest(
    rgcx.full,
    data = rd.na,
    labpos = c(0.1, 10),
...
```{r survplota5v5, echo = FALSE, fig.width = 12, fig.height = 7, fig.cap = "Survival plot showing CRT-free time for five-lumen central venous catheters."}
...
```{r survplota4a5, echo = FALSE, fig.width = 12, fig.height = 7, fig.cap = "Survival plot showing CRT-free time for four- and five-lumen central venous catheters."}
xlim <- c(0.8, max(times))
mar <- c(0.1, 10.1, 1.1, 2.1)
...
knitr::kable(
    lottbl,
    col.names = c(
        "Id", "Manufacturer", "Type", "Lumens", "LOT", "PN", "Excluded"
    ),
    caption = "LOT and part numbers (PN) of examined central venous catheters."
)
```
