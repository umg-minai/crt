...
We also had to exclude `r sum(cvc$ExclusionReason == "Missing LOT", na.rm = TRUE)` CVCs due to missing or unreadable LOT information (see Supplementary Information, Table \@ref(tab:lottable), for a full list).
...
Due to catheter removal or ICU discharge, we were only able to perform follow-up ultrasound examinations for `r .tbl1("FollowUp")` days.

Specifically, we found CRTs in `r crt <- sort(1 - .survival(m$svf, times = 1)); crt <- paste0(round(crt * 100), "% of all ", sub("^ManLu=", "", names(crt))); .cli_text("{ {crt}}")` CVCs at the first examination (within 24 hours; see also Fig. \@ref(fig:survplot)).
...
However, the median (IQR) largest dimension of the thrombosis was almost identical across all examinations: `r .medLU(m$data$HeightMax)` mm and `r .medLU(m$data$WidthMax)` mm, respectively (see Supplementary Information, Fig. \@ref(fig:crtheightbp) to \@ref(fig:crtwidthovertime) for detailed figures).
...
`r round(p.adjust(m.a4a5$svd$pvalue, method = "BH", n = ncmb), 2)` for Arrow4 and `r round(p.adjust(m.a5v5$svd$pvalue, method = "BH", n = ncmb), 2)` for Vygon5 (see the Supplementary Information for survival plots of these comparisons, Fig. \@ref(fig:survplota5v5) and \@ref(fig:survplota4a5)).
...
In addition to the univariate analyses we applied multiple regression analyses to determine the impact of different variables (see Fig. \@ref(fig:forestplot)).
All variables with missing values were omitted to maintain the sample size.
However, the results don't vary much even when all variables are included (see Supplementary Information for a regression with half the number of samples but all the variables, Table \@ref(tab:supplhrtablefull) and Fig. \@ref(fig:forestplotfull)).

As shown in the forest plot (see Fig. \@ref(fig:forestplot)), the Arrow4 and Vygon5 CVC types were significantly associated with higher rates of CRT.
...
However, men had a significant less risk of CRT (hazard ratio `r format(exp(rgcx$coefficients["Sexmale"]), digits = 2)`) than women.
