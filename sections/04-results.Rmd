...
However, the median (IQR) largest dimension of the thrombosis was almost identical across all examinations: `r .medLU(m$data$HeightMax)` mm and `r .medLU(m$data$WidthMax)` mm, respectively (see Supplement \@ref(fig:crtheightbp) to \@ref(fig:crtwidthovertime) for detailed figures).
...
Lumen occlusion was reported in `r s <- sum(m$data$Complications == "closed lumen", na.rm = TRUE); .cli_text("{(.as.word(s))} {qty(s)}CVC{?s} {qty(s)}{?was/were}")`.
